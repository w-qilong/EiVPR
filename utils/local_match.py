...
def rerank_function(predictions,queries_local_features,database_local_features):
    pred2 = []
    print("reranking...")
    for query_index, pred in enumerate(tqdm(predictions)):
        ...
        rerank_index = local_sim(query_local_features,positives_local_features).cpu().numpy().argsort()[::-1].copy()
...
if __name__ == '__main__':
    a = torch.randn((2, 61, 61, 768)).cuda()
    b = torch.randn((2, 61, 61, 768)).cuda()
    c = local_sim(a, b, trainflag=True)
    print(c.shape)
